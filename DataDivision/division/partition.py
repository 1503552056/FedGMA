...
class TxtPartitioner(DataPartitioner):
    num_classes = 2  

    def __init__(self, txt_dir, num_clients,
                 partition='iid',
                 dir_alpha=None,
                 major_classes_num=1,
                 verbose=True,
        ...
        self.paths, self.targets = self._load_txts()
        ...
        if partition == "noniid-#label":
            ...
            assert dir_alpha is not None and dir_alpha > 0
        elif self.partition == "noniid-quantitydir":
            assert dir_alpha > 0
        elif partition != "iid":
        ...
        self.stats_report = F.partition_report(self.targets, self.client_dict, class_num=self.num_classes, verbose=False)

    def _load_txts(self):
        paths = []
        labels = []
        for file in glob(os.path.join(self.txt_dir, "*.txt")):
    ...
    def _perform_partition(self):
        if self.partition == "noniid-#label":
            return F.label_skew_quantity_based_partition(
                self.targets, self.num_clients, self.num_classes, self.major_classes_num)

        elif self.partition == "noniid-labeldir":
            return F.hetero_dir_partition(
                self.targets, self.num_clients, self.num_classes, self.dir_alpha, self.min_require_size)

        elif self.partition == "unbalance":
            client_sample_nums = F.dirichlet_unbalance_split(
                self.num_clients, self.num_samples, self.dir_alpha)
            return F.homo_partition(client_sample_nums, self.num_samples)
        
        elif self.partition == "noniid-quantitydir":
            client_sample_nums = F.balance_split(self.num_clients, self.num_samples)
            return F.client_inner_dirichlet_partition(
                self.targets, self.num_clients, self.num_classes, self.dir_alpha, client_sample_nums)
    ...
    def save(self, output_dir):
        ...
        for cid, indices in self.client_dict.items():
            with open(os.path.join(output_dir, f"client_{cid}.txt"), "w") as f:
                for idx in indices:
                    f.write(f"{self.paths[idx]} {self.targets[idx]}\n")
